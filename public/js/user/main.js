...
document.getElementById('cancel-upload').addEventListener('click', async(e)=>{
...
async function showUserData(){
    ...
    total.innerText = earning.total;
    today.innerText = earning.today;
    direct.innerText = earning.direct;
    level.innerText = earning.level;
    ...
    const notActiveTeam = members.team.filter((member)=> member.planType === null);
...
function showTeam(total, active, notActive){
...
document.getElementById('upload').addEventListener('click', async (e) =>{
    ...
    try{
        ...
        const res = await axios.post("http://localhost:4000/main/joining-request", formData, {
